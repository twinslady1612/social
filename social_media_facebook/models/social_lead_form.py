...
class SocialLeadForm(models.Model):
    ...
    post_account_id = fields.Many2one(
        "social.post.account",
        string="Post Account",
        help="The specific social.post.account (ad on a Facebook page) using this lead form",
        ondelete="set null",
    )
    ...
    def _process_facebook_lead_data(self, lead_data):
        ...
        # Auto-create field mappings if this is the first lead
        if not self.field_mapping_ids and field_data:
            self._auto_create_field_mappings(field_data)

        # Don't auto-create CRM lead - let user manually convert after reviewing field mappings
        # Users can click "Convert to CRM Lead" button on the social.lead form view

        return social_lead

    def _auto_create_field_mappings(self, field_data):
        """Auto-create field mapping records from Facebook form fields

        Args:
            field_data: List of field dictionaries from Facebook lead
                       [{"name": "email", "values": ["test@example.com"]}, ...]
        """
        self.ensure_one()

        # Common Facebook field -> CRM field mappings
        # Keys can be uppercase (from form questions API) or lowercase (from lead data)
        default_mappings = {
            "email": "email_from",
            "EMAIL": "email_from",
        ...
        mapping_vals = []
        for field in field_data:
            fb_field_name = field.get("name", "")
            if not fb_field_name:
                continue

            # Check if mapping already exists
            existing = self.field_mapping_ids.filtered(
                lambda m: m.platform_field_name == fb_field_name
            )
            if existing:
                continue

            # Get default CRM field mapping or leave empty for user to fill manually
            crm_field = default_mappings.get(fb_field_name, False)

            mapping_vals.append({
                "lead_form_id": self.id,
                "platform_field_name": fb_field_name,
                "crm_field_name": crm_field if crm_field else False,
            })

        if mapping_vals:
            self.env["social.lead.field.mapping"].create(mapping_vals)
            print(f"Auto-created {len(mapping_vals)} field mappings for form {self.name}")
            print(f"Please review and update the CRM field mappings in the form settings")
...
class SocialLead(models.Model):
    ...
    def _create_facebook_crm_lead(self):
        ...
        try:
            ...
            # Apply custom field mappings (skip unmapped fields where crm_field_name is False)
            for mapping in self.lead_form_id.field_mapping_ids:
                if not mapping.crm_field_name:
